    ...
    function invenioSearchSuccessHandler(event, response) {
      ...
      if (response.data.links) {
        ...
        if (!angular.equals(vm.invenioSearchCurrentArgs, data)) {
          vm.invenioSearchCurrentArgs.params = angular.merge(
            {},
  ...
  function invenioSearchFacets() {
    ...
    function link(scope, element, attrs, vm) {
      ...
      function handleClick(key, value) {
        // Get current
        var current = vm.invenioSearchCurrentArgs.params[key] || [];
    ...
    function link(scope, element, attrs, vm) {
      ...
      function isSelected(value) {
        ...
        var check = (
          ...
          vm.invenioSearchCurrentArgs.params[scope.data.sortKey] || ''
        );
        if (check.charAt(0) === '-'){
      ...
      scope.$watchCollection(
        'vm.invenioSearchCurrentArgs.params.sort', onCurrentSearchChange
      );
      scope.$watchCollection(
